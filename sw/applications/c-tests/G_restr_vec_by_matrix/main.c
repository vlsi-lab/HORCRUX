...
#define ENABLE_restr_vec_by_fp_matrix_G 1
...
FP_DOUBLEPREC g_restr_vec_by_matrix(FP_DOUBLEPREC a, FP_DOUBLEPREC b, FP_DOUBLEPREC c)
{
    FP_DOUBLEPREC out;
    asm volatile (
        "addi t0, %1, 0\n\t"                
        ".insn r 0x6b, 0x05, 0x2, %0, t0, %2, %3\n\t"
...
int main() {
    ...
    FP_DOUBLEPREC e[K] = { 70, 111, 70, 111, 70 };

    FP_DOUBLEPREC V_tr[K][N_K] = {
        {
        54, 61, 333, 454, 489, 465, 347, 278, 465, 176
        },
        { /* i = 1 (e=111) */
        295, 391, 248, 205, 107, 284, 266, 467, 350, 218
        },
        {
        54, 61, 333, 454, 489, 465, 347, 278, 465, 176
        },
    ...
    FP_DOUBLEPREC res[K][N_K] = {
        { /* inputs for i=0 (your 'input[...]') */
        104, 104, 350, 62, 224, 281, 148, 97, 460, 145
        },
        { /* inputs for i=1 (your 'input[...]') */
        393, 44, 181, 41, 494, 366, 299, 416, 36, 314
        },
        { /* inputs for i=0 (your 'input[...]') */
        104, 104, 350, 62, 224, 281, 148, 97, 460, 145
        },
    ...
    FP_DOUBLEPREC golden_res[K][N_K] = {
        ...
        { /* RED results for i=1 */
        426, 438, 138, 340, 223, 13, 101, 99, 472, 190
        }, 
        { /* RED results for i=0 */
        393, 44, 181, 41, 494, 366, 299, 416, 36, 314
        },
    ...
    for(int i = 0; i < K; i++){
        for(int j = 0; j < N_K; j++){
            computed_res[i][j] = FPRED_DOUBLE( (FP_DOUBLEPREC) res[i][j] +
    ...
    for (int i = 0; i < K; i++) {
        for (int j = 0; j < N_K; j++) {
            ...
            //"mv a3, %[res]\n\t"
            //"mv a5, %[V_tr]\n\t"
            //".insn r 0x6b, 0x05, 0x2, %[rd], %[e], a5, a3\n\t"
            //: [rd] "=r" (computed_res_hw[i][j])
            //: [e] "r" (e[i]), [V_tr] "r" (V_tr[i][j]), [res] "r" (res[i][j])
            //: "cc", "a3", "a5");
            computed_res_hw[i][j] = g_restr_vec_by_matrix(e[i], V_tr[i][j], res[i][j]);
        }
    }
    CSR_READ(CSR_REG_MCYCLE, &cycles);
