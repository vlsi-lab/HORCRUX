...
uint32_t rej12x2 (uint32_t a, uint32_t b, uint32_t c)
{
    uint32_t out;
    asm volatile (
        ".insn r 0x6b, 0x2, 0, %0, %1, %2, %3\n\t"
...
int main(void)
{
    ...
    for (int i = 0; i < NTESTS_R12; ++i) {
        //asm volatile (
        //    ".insn r 0x6b, 0x02, 0x0, %[dst], %[rs1], %[rs2], %[rs3]\n\t":
        //    [dst] "=r" (got_hw[i]) :
        //    [rs1] "r" (rs1_in[i]), [rs2] "r" (0), [rs3] "r" (m)
        //    :
        //);
        got_hw[i] = rej12x2(rs1_in[i], 0, m);
    }

    #if PERF_CNT_CYCLES == 1
        CSR_READ(CSR_REG_MCYCLE, &cycles);
        printf("test_rej12x2_hw: %d\n", cycles);
    #endif

    for (int i = 0; i < NTESTS_R12; ++i) {
        if (got[i] != golden[i]) {
            printf("REJ12x2 FAIL [%d]: rs1=0x%08X exp=0x%08X got=0x%08X\n",
                    i, rs1_in[i], golden[i], got_hw[i]);
            pass = 0;
        }
    }
