...
int16_t montgomery_reduce_ntrup_hw(int32_t a)
{
    int32_t out;
    asm volatile (
        "addi t0, %1, 0\n\t"                // t0 := rs1 copy
...
int main(void) {
    ...
    for (int i = 0; i < N_TESTS; i++) {
        ...
        //    : [a] "r" (ntrup_inputs[i])
        //    : "a3", "cc" );
        got_hw[i] = montgomery_reduce_ntrup_hw(ntrup_inputs[i]);
    }
    ...
    for (int i = 0; i < N_TESTS; i++) {
        if (got_hw[i] != ntrup_golden[i]) {
            printf("[NTRU-P] Test %2d FAIL: a=%d  exp=%d  got=%d\n",
                    i, ntrup_inputs[i], ntrup_golden[i], got_hw[i]);
            kem_ok_hw = 0;
            }
        }
        
    if (kem_ok_hw) printf("[NTRU-P] All 20 Montgomery tests PASSED.\n");
