...
void synd_minus_vec_scaled(FP_DOUBLEPREC a, FP_DOUBLEPREC b, FP_DOUBLEPREC c, FP_DOUBLEPREC *result) {
        
    asm volatile (
        "addi t0, %1, 0\n\t" 
        ".insn r 0x6b, 0x04, 0x0, %0, t0, %2, %3\n\t"
        : [rd] "=r" (*result)
...
int main() {
    ...
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    for (int i = 0; i < N_K; i++){  
        synd_minus_vec_scaled(s[i], chall_1, synd[i], &computed_res_hw[i]);
    }
    CSR_READ(CSR_REG_MCYCLE, &cycles);
