...
uint32_t rej23x1 (uint32_t a, uint32_t b, uint32_t c)
{
    uint32_t out;
    asm volatile (
        ".insn r 0x6b, 0x2, 1, %0, %1, %2, %3\n\t"
...
int main(void)
{
    ...
    for (int i = 0; i < NTESTS_R23; ++i) {
        got_hw[i] = rej23x1(rs1_in[i], 0, m);
        //asm volatile (
        //    ".insn r 0x6b, 0x02, 0x1, %[dst], %[rs1], %[rs2], %[rs3]\n\t":
        //    [dst] "=r" (got_hw[i]) :
